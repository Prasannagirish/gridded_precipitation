...
# ==========================================================
#  3. SYNTHETIC UNIT HYDROGRAPH (Snyder's Method)
#
#  Snyder's method defines the UH based on basin geometry:
#    tp = Ct * (L * Lc)^0.3  (Lag time)
#    qp = (Cp * 640) / tp    (Peak discharge)
#
#  We use a Gamma function to distribution the volume.
# ==========================================================
print("\n" + "=" * 60)
print("  3. SYNTHETIC UNIT HYDROGRAPH (Snyder's)")
print("=" * 60)

from scipy.special import gamma

# --- Physical Inputs (Kabini Upstream Estimates) ---
L  = 85.0    # Longest flow path (km)
Lc = 42.0    # Length to centroid along main stream (km)
Ct = 2.0     # Regional coefficient for lag (1.8 - 2.2 for Western Ghats)
Cp = 0.65    # Regional coefficient for peak (0.4 - 0.9)

# 1. Calculate Basin Lag (hours)
tp_hours = Ct * (L * Lc)**0.3
tp_days  = tp_hours / 24

# 2. Calculate Peak Discharge (m3/s per mm per km2)
# Snyder's standard formula is in sq miles; adjusted for Metric:
# qp (m3/s/km2/mm) = 0.275 * Cp / tp_days (approx)
qp_specific = (0.275 * Cp) / tp_days 
Qp_total_uh = qp_specific * CATCHMENT_AREA_KM2

# 3. Time Base (Tb) in days
Tb_days = 3 + 3 * (tp_days / 24) # Standard Snyder approximation

print(f"   Basin Lag (tp):    {tp_hours:.2f} hours ({tp_days:.2f} days)")
print(f"   Peak Flow (Qp):    {Qp_total_uh:.4f} (m³/s)/mm")
print(f"   Time Base (Tb):    {Tb_days:.2f} days")

t_uh = np.linspace(0, Tb_days, 100)
# Gamma shape parameters to match Snyder's Qp and tp
beta = tp_days / 2  # Adjusts peak location
alpha = (Qp_total_uh * beta * gamma(2)) / tp_days # Normalization factor (simplified)
# Refined Gamma approach for exact unit volume:
def snyder_shape(t, tp, qp):
    # Standard dimensionless shape: (t/tp)^a * exp(a * (1 - t/tp))
    a = 3.5 # Shape factor for typical natural basins
    return qp * (t/tp)**a * np.exp(a * (1 - t/tp))

uh_values = snyder_shape(t_uh, tp_days, Qp_total_uh)

current_vol = trapezoid(uh_values, t_uh) * 86400 / (CATCHMENT_AREA_M2) * 1000
uh_values = uh_values / current_vol # Force volume to exactly 1.0 mm

print(f"   Verified UH Volume: {np.trapezoid(uh_values, t_uh)*86400/(CATCHMENT_AREA_M2)*1000:.1f} mm")
...
ax.annotate(f'Peak Lag (tp)\n{tp_hours:.1f} hrs', xy=(tp_days, Qp_total_uh/2), 
            xytext=(tp_days+1, Qp_total_uh/2), arrowprops=dict(arrowstyle='->'))

ax.set_title(f"Unique Synthetic Unit Hydrograph: Kabini Basin")
ax.set_xlabel("Time (days)")
ax.set_ylabel("Discharge (m³/s) per 1mm Excess Rain")
ax.grid(True, alpha=0.3)
ax.legend()
plt.tight_layout()
plt.savefig(HYDRO_DIR / "snyder_unit_hydrograph.png", dpi=150)
plt.close()

print(f"   📸 Saved: snyder_unit_hydrograph.png")

# Update global vars for later sections
Tp_uh, Qp_uh, Tb_uh,Tc_uh = tp_days, Qp_total_uh, Tb_days,tp_days
